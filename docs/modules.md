...
| `settings_ui.py` | settings dialog (booleans, ui_scale, columns...; "Duplicate check" section with the manual scan button; "Profiles" section for switching/creating/renaming/editing/deleting profiles; hides Last.fm / SoundCloud sections when those plugins are not installed, gated by the `plugin_availability` ids passed in) | `main_window` | `settings_theme_ui`, `profiles_ui`, `services.profile_store`, `ui.scrollable`, `utils.{config,scaling,theme}` |
...
| `scaling.py` | `init(root)` / `ui_font()` / `px()`: single source of scale factor; must run before any widget | `app.app` (first), then icons, theme, all UI | `utils.config` |
