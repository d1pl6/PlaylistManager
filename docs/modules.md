...
| `app/main.py` | argparse dispatch; `-a/-p/--list/login/...` go to `cli`, default starts GUI; `--data-saver` forces the "max" thumbnail mode for the run; `--fullscreen` forces the `[fullscreen]` setting for the run; `--start-in-tray` forces the `[start_in_tray]` setting for the run | `main.py`, `app/__main__.py` | `utils.logging_config`; `app.app` via `_import_app()` (handles package/module dual identity) |
| `app/__main__.py` | Makes `python -m app` work | interpreter | `app.main` |
| `app/app.py` | `App`: boot sequence (see flows.md #1), auth bootstrap, quit path; sets `utils.thumbnail.DATA_SAVER` from `--data-saver` before any widget exists; restores the saved `[window] geometry` before centering (restored geometry skips both centering sites), applies fullscreen (setting or `--fullscreen`), binds the debounced `<Configure>` geometry save + window-local F11 fullscreen toggle, flushes geometry in `cleanup()`, withdraws before any setup work when `start_in_tray`/`--start-in-tray` is effective (the window never maps — no flash; deiconified only if the tray failed to start), seeds the geometry no-op cache after setup (first-run defaults are never persisted) | `app.main._import_app()` | `plugin_loader`, `services.integration`, `services.playlist_store`, `services.tray`, `controllers.app_controller`, `controllers.keybind_controller`, `ui.main_window`, `ui.updater_ui`, `utils.updater`, `utils.{config,scaling,window,logging_config}` |
| `app/cli.py` | Headless commands (incl. `-a` batch adds, which bump the grid "Last used" timestamp via `PlaylistStore.mark_used` on each `status == "added"` result); shares the service layer, never touches tkinter | `app.main` | `plugin_loader`, `services.{database,integration,playlist_store,playlist_sync,playlist_url,song_manager,auth_setup,scrobble_log,scrobble}`, `utils.{thumbnail,logging_config}` |
...
| `keybind_controller.py` | pynput listener lifecycle, keybind recording state machine, per-platform flow init (`_ensure_initialized` -> lock-serialized `_build_flow`, receiver registered only after flow construction succeeds), public accessors `get_flow` + `flow_busy` for the activity window's Add worker (which holds the busy lock around `execute_flow` so captures cannot race keybind/scrobble flows), dispatch (`handle_keybind`) — on a `status == "added"` result also `PlaylistStore.mark_used()` so the grid "Last used" sort sees live adds; credential swap (`update_credentials`) | `app.app` (constructed); `ui.card_grid` / `ui.main_window` hand it callbacks | `controllers.keybind_registry`, `services.{duplicate_queue,playlist_store,song_manager,scrobble_log,scrobble}`, `utils.{key_mapping,platform,theme}` |
...
| `scrobble.py` | Per-platform auto-scrobble sources: `split_platforms()` parses the `[scrobble] platforms` CSV; `scrobble_enabled_for(pid)` = master `scrobble_on_add` AND pid allowed — empty storage means ALL installed (historical default), literal `"none"` (SCROBBLE_NONE sentinel) means NONE, otherwise an explicit id list. The Settings checklist writes the explicit list / `"none"` on uncheck-all | `keybind_controller` (hotkey add-flow gate at the scrobble call site), `app.cli` (backend resolution + per-target gate), `ui.settings_ui` | `utils.config` |
| `playlist_store.py` | `db/playlists.json` (profile-aware) CRUD, thread-safe, TTL cache; `ensure_playlists_file()`, `migrate_schema()`; `mark_used()` stamps the entry's `last_used_at` (grid "Last used" sort, only on real adds — no-op when the entry is gone), `set_pinned()` flips the per-playlist `pinned` flag (unpin removes the key). New entries omit both fields — no migration needed; `add_playlist` updates in place so they survive a same-plain-id re-add | `app.app`, `app.cli`, `keybind_controller`, `playlist_controller`, `card_grid`, `main_window`, `showcase_manager`, `playlist_sync`, integrations' `flow.py` | stdlib only; `services.profile_store` |
| `playlist_sort.py` | Pure grid ordering for the main window: `sort_playlists(entries, key, direction)` over the registry list. Keys (`GRID_SORT_KEYS` in `utils.config`): `name`/`platform` (case-insensitive), `added` (original list index = insertion order, captured before any partition), `used` (`last_used_at`, never-used entries sorted to the end in both directions). Pinned playlists partition to the top, then the active key applies within each group. Stable — equal keys keep input order; unknown keys fall back to `name` | `ui.main_window` | `utils.config` (constants) |
| `playlist_sync.py` | `PlaylistSyncService`: threaded `import_tracks` / `reload_database` (+ `_sync` cores run in worker threads); after each import/reload `ThumbnailService.warm_song_cache()` prefetches every song thumb in "download" mode (background thread) | `ui.main_window`, `app.cli` | `services.{database,playlist_store,song_manager}`, `utils.thumbnail` |
...
| File | Role | Used by | Uses |
|---|---|---|---|
| `main_window.py` | Composition root: toolbar (login, Add, Settings/Activity buttons; sort cluster moved to `settings_ui`), card grid, search, showcase rows, dialog wiring; implements the UI side of `KeybindCallbacks`; activity-window host (badge poll, song dispatcher, Add worker, duplicate scanner); import/reload `on_done` rebuilds the grid and lets the showcase prune the song cache ("cache" mode). Grid population lives in `_populate_grid(entries)` (sorted via `sort_playlists`; called from `setup()`); `_rebuild_grid()` re-sorts in place after sort/pin changes — deliberately NON-destructive (never `close_main_frame`: it deletes registry entries), it unregisters keybinds, tears frames down, fresh-reads the registry, re-applies the active search query  | `app.app` | `activity_window`, `card_grid`, `playlist_controller`, `login_ui`, `playlist_dialog`, `search_manager`, `showcase_manager`, `settings_ui`, `scrollable`, `tooltip`, `keybind_registry`, `services.{database,duplicate_check,duplicate_queue,playlist_sort,playlist_store,playlist_sync,song_manager}`, `utils.{config,icons,platform,scaling,theme,window}` |
| `card_grid.py` | `CardGridManager`: grid of cards, per-card actions, incl. the playlist ✕ (`_confirm_close_playlist` — `resolve_playlist_removal_mode()` picks "remove"/"keep_db" by `confirm_on_playlist_remove` + `[remove_playlist] default`, showing the Cancel/Keep-DB/Confirm dialog only when the ask is on) and the **pin/unpin toggle** (small icon button left of the ✕ in a `header_actions` sub-frame; `pin.png`/`pin_active.png` swap via `set_card_pin_state`, tooltip reads the button's `_pm_pinned`; command captures the CARD not the index — same rule as the close button; `_apply_pin_visibility` hides/shows without touching stored flags; `reset_state()` clears cards + the parallel `_pin_buttons`/`_close_buttons`/`_header_actions` lists on rebuilds or stale widgets hit bad window paths); receives a `make_keybind_callbacks(card_num)` factory, never builds flows itself | `main_window` | `ui.{card,close_playlist_dialog,scrollable,tooltip}`, `controllers.keybind_registry`, `services.{database,playlist_store,scrobble_log}`, `utils.{config,scaling,theme}` |
| `card.py` | `PlaylistCard` dataclass + card widget | `card_grid` | stdlib/tkinter |
| `showcase_manager.py` | last-N-songs row per card; song-aware thumb fetch (`ThumbnailService.fetch_song_image`, mode-driven); tracks per-card rendered song URLs and prunes the disk cache after each full rebuild in "cache" mode; remove-song ✕ flow — platform-first, always full (local delete + scrobble wrangling), gated by an optional in-app yes/no confirmation dialog (`_ask_confirm_remove_song`, modal, theme-built); no "keep db" option for songs | `main_window` | `services.{playlist_store,song_manager,scrobble_log}`, `utils.{icons,thumbnail,scaling,theme,config}`, `ui.tooltip` |
...
| `settings_ui.py` | settings dialog (booleans, ui_scale, columns, font family, Thumbnails mode combobox + "Clear thumbnail cache" button with live cache-size readout, duplicate check + profiles sections; "Ask before removing a song" — plain yes/no confirmation for the showcase ✕; "Ask before removing the playlist" checkbox + default-playlist-removal combobox (locked while the confirmation is on); "Pin/unpin buttons (on playlist cards)" checkbox calling `on_pin_buttons_change` (main_window's `_apply_pin_visibility`) live; "Sort playlists by:" combobox (Name/Platform/Added/Last used) + direction arrow button writing `[grid_sort] key/direction` live via `on_sort_change`; per-platform scrobble checklist — "Platforms to scrobble from:" checkboxes for every installed music platform (lastfm excluded — it's the target) packed only while `scrobble_on_add` is on, "Install integration first" placeholder when only Last.fm is installed, writes `[scrobble] platforms` (explicit list, or the `"none"` sentinel when all unchecked); "Hide in tray on minimize" and "Start in tray (next launch)" rows gated on tray availability; hides Last.fm / SoundCloud sections when those plugins are not installed, gated by the `plugin_availability` ids passed in; `platform_display_names` mapping feeds the checklist labels) | `main_window` | `settings_theme_ui`, `profiles_ui`, `services.profile_store`, `services.scrobble`, `ui.scrollable`, `utils.{config,scaling,theme,thumbnail}` |
...
| `scrollable.py` | `ScrollableFrame` mixin (optional `max_viewport_height` caps the canvas height for nested scroll regions — used by the per-platform scrobble checklist inside the scrolling settings dialog) | `card_grid`, `playlist_dialog`, `settings_ui`, `settings_theme_ui`, `main_window` | `utils.theme` |
...
| `thumbnail.py` | `ThumbnailService.fetch_image()` (covers, any thread) / `fetch_song_image()` (song rows, mode-aware) / `fetch_full_image()` (originals) / `to_photoimage()` (main thread only). Data-saver modes (`[thumbnails] mode`, `utils.config.THUMBNAIL_MODES`): `off` live-fetch in-memory only; `download` persists every fetched image under `platformdirs.user_cache_dir("playlistmanager")` (`~/.cache/playlistmanager/` on Linux; shared across profiles, subsystem paths `playlists/`, `songs/`, `full/`) + `warm_song_cache()` prefetch; `dedupe` keys song thumbs by identity via the duplicate-check matcher through `songs/index.json` (fuzzy when `[duplicate_check] is_true`, exact at 1.0 threshold otherwise; never touches add-path policy); `cache` keeps covers + visible showcase thumbs and `prune_song_cache()` deletes the rest (also drops orphaned index rows); `max` returns None for every fetch (forced by `DATA_SAVER`, set from `--data-saver`). `clear_disk_cache()` / `disk_cache_size()` feed the Settings clear button | `playlist_dialog`, `showcase_manager`, `playlist_sync`, `app.cli` | requests, Pillow; `services.{song_manager,duplicate_check}` (lazy), `utils.config` |
...
| `window.py` | `center_window` / `resize_window` / `fit_window_to_screen` (caps a Toplevel at the screen size, used before centering so over-tall dialogs scroll instead of opening off-screen), pure geometry; window-geometry persistence: `save_window_geometry` (resolved `winfo_*` numbers, skips maximized/iconic/withdrawn/unmapped/fullscreen), `restore_window_geometry` (parse -> clamp size to screen -> visibility check -> apply; absolute negative positions written as `+-N` because `-N` means right-edge in Tk), display-free `_parse_geometry`/`_clamp_rect`/`_visible` validated headlessly | `app.app` and most dialogs | tkinter only |
