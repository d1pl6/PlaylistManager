...
| `app/main.py` | argparse dispatch; `-a/-p/--list/login/...` go to `cli`, default starts GUI; `--data-saver` forces the "max" thumbnail mode for the run | `main.py`, `app/__main__.py` | `utils.logging_config`; `app.app` via `_import_app()` (handles package/module dual identity) |
| `app/__main__.py` | Makes `python -m app` work | interpreter | `app.main` |
| `app/app.py` | `App`: boot sequence (see flows.md #1), auth bootstrap, quit path; sets `utils.thumbnail.DATA_SAVER` from `--data-saver` before any widget exists | `app.main._import_app()` | `plugin_loader`, `services.integration`, `services.playlist_store`, `services.tray`, `controllers.app_controller`, `controllers.keybind_controller`, `ui.main_window`, `ui.updater_ui`, `utils.updater`, `utils.{config,scaling,window,logging_config}` |
