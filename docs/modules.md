...
| `showcase_manager.py` | last-N-songs row per card; song-aware thumb fetch (`ThumbnailService.fetch_song_image`, mode-driven); tracks per-card rendered song URLs and prunes the disk cache after each full rebuild in "cache" mode | `main_window` | `services.{playlist_store,song_manager,scrobble_log}`, `utils.{icons,thumbnail,scaling,theme}`, `ui.tooltip` |
