...
- **Quit path**: never join the pynput listener thread. `listener.stop()` alone stops event delivery; on this Linux setup the thread stays alive until process exit (daemon), and `join(timeout=N)` blocks the caller for the **full** N seconds. Teardown must call `kc.stop_listener(wait=False)` — a `wait=True` join is only acceptable on the Settings→toggle-listener path, and keep it short (≤0.5 s). See `app_controller`/`MainWindow.cleanup()` for the established pattern.
